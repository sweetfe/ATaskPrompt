...
### Intuitive Gestures
- Left swipe to skip/decline a task
- Right swipe to accept/complete a task
...
const TaskSwipe = ({ task, onSwipeLeft, onSwipeRight }) => {
  const handlers = useSwipeable({
    onSwipedLeft: () => onSwipeLeft(task.id),
    onSwipedRight: () => onSwipeRight(task.id),
...
#### Left Swipe (Skip/Decline)
- **Gesture**: Swipe from right to left
- **Action**: Skip the current task
- **Visual Feedback**: Card moves left with red overlay
- **Outcome**: Task is marked as skipped, next task is shown

#### Right Swipe (Accept/Complete)
- **Gesture**: Swipe from left to right
- **Action**: Accept and complete the current task
- **Visual Feedback**: Card moves right with green overlay
...
- **Left Arrow**: Skip current task
- **Right Arrow**: Accept current task
- **Up Arrow**: Show task details
- **Down Arrow**: Dismiss current view
- **Space**: Trigger current action
